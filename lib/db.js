...
ensureColumn("cases", "breach_check", "breach_check TEXT");
// Engineer -> victim contact, delivered inside the platform (no email needed).
ensureColumn("cases", "engineer_message", "engineer_message TEXT NOT NULL DEFAULT ''");
ensureColumn("cases", "meeting_link", "meeting_link TEXT NOT NULL DEFAULT ''");
ensureColumn("cases", "engineer_message_at", "engineer_message_at TEXT");
ensureColumn("feedback", "amount", "amount INTEGER NOT NULL DEFAULT 0");
ensureColumn("feedback", "trx_id", "trx_id TEXT");
ensureColumn("feedback", "payment_status", "payment_status TEXT NOT NULL DEFAULT ''");
// Prototype identity check (KYC demo) — status only, never any image.
ensureColumn("users", "kyc_status", "kyc_status TEXT NOT NULL DEFAULT 'none'");
ensureColumn("users", "kyc_verified_at", "kyc_verified_at TEXT");
// Email verification, offered alongside phone OTP as a verification choice.
ensureColumn("users", "email_verified", "email_verified INTEGER NOT NULL DEFAULT 0");
// Which channel an OTP was sent on ("phone" | "email").
ensureColumn("otp_codes", "channel", "channel TEXT NOT NULL DEFAULT 'phone'");
...
function rowToCase(row) {
  ...
  return {
    ...
    breachCheck: row.breach_check ? JSON.parse(row.breach_check) : null,
    engineerMessage: row.engineer_message ?? "",
    meetingLink: row.meeting_link ?? "",
    engineerMessageAt: row.engineer_message_at ?? null,
...
export function getOrCreateCase(sessionId, userId = null) {
  ...
  addCaseEvent(sessionId, "created");
...
export function appendTurn(sessionId, conversation, parsedReply) {
  // Stamp the moment intake finishes so the victim's progress timeline
  // shows exactly when their case reached the engineer queue.
  const prev = db.prepare("SELECT status FROM cases WHERE id = ?").get(sessionId);
  const newStatus = parsedReply.status ?? "collecting";
  if (
    prev &&
    prev.status !== "complete" &&
    prev.status !== "blocked_minor" &&
    (newStatus === "complete" || newStatus === "blocked_minor")
  ) {
    addCaseEvent(sessionId, "submitted");
  }
...
const CASE_JOIN = `
  SELECT c.*, u.name AS u_name, u.email AS u_email,
         ...
         u.email_verified AS u_email_verified, u.kyc_status AS u_kyc_status
...
// Store the breach/leak lookup result on the case so the engineer sees it.
// `result` is the sanitized object the API already built (query, status,
// breaches, source, checkedAt) — we never store passwords or raw API errors.
export function saveBreachCheck(id, result) {
  db.prepare(
    `UPDATE cases SET breach_check = ?, updated_at = ? WHERE id = ?`,
  ).run(JSON.stringify(result), now(), id);
  addCaseEvent(id, "breach_checked");
...
export function updateCaseWorkflow(id, { caseStatus, engineerNotes }) {
  ...
  const prev = db.prepare("SELECT case_status FROM cases WHERE id = ?").get(id);
  db.prepare(
    `UPDATE cases SET case_status = ?, engineer_notes = ?, updated_at = ? WHERE id = ?`,
  ).run(caseStatus, String(engineerNotes ?? "").slice(0, 5000), now(), id);
  if (prev && prev.case_status !== caseStatus) {
    addCaseEvent(id, caseStatus);
  }
  return getCaseById(id);
}

// Engineer's message + optional meeting link shown to the victim in her chat.
// This is how the engineer reaches the victim — entirely inside the platform.
export function setEngineerContact(id, { engineerMessage, meetingLink }) {
  const msg = String(engineerMessage ?? "").slice(0, 2000);
  const link = String(meetingLink ?? "").slice(0, 500);
  const prev = db.prepare("SELECT engineer_message FROM cases WHERE id = ?").get(id);
  db.prepare(
    `UPDATE cases SET engineer_message = ?, meeting_link = ?,
       engineer_message_at = ?, updated_at = ? WHERE id = ?`,
  ).run(msg, link, msg ? now() : null, now(), id);
  // Record an event the first time (or when changed) so there's an audit trail.
  if (msg && (!prev || prev.engineer_message !== msg)) {
    addCaseEvent(id, "engineer_message");
  }
  return getCaseById(id);
}
...
export function addCaseEvent(caseId, type) {
  db.prepare(
    `INSERT INTO case_events (id, case_id, type, created_at) VALUES (?, ?, ?, ?)`,
  ).run(randomUUID(), caseId, type, now());
}

export function listCaseEvents(caseId) {
  return db
    .prepare(
      `SELECT type, created_at FROM case_events WHERE case_id = ? ORDER BY created_at ASC`,
    )
    .all(caseId)
    .map((row) => ({ type: row.type, at: row.created_at }));
}

/* -------------------------- case evidence ------------------------- */
// Screenshots the victim attaches as evidence (fake profiles, scam chats,
// threats). NEVER intimate images — that rule lives in the UI copy. Files are
// stored on disk under data/uploads/<case>/; only metadata lives here.

export function addEvidence(caseId, { id, file, mime, size }) {
  db.prepare(
    `INSERT INTO case_evidence (id, case_id, file, mime, size, created_at)
     VALUES (?, ?, ?, ?, ?, ?)`,
  ).run(id, caseId, file, mime, size, now());
...
export function listEvidence(caseId) {
  return db
    .prepare(
      `SELECT id, file, mime, size, created_at FROM case_evidence
       WHERE case_id = ? ORDER BY created_at ASC`,
    )
    .all(caseId)
    .map((r) => ({ id: r.id, file: r.file, mime: r.mime, size: r.size, at: r.created_at }));
}

export function countEvidence(caseId) {
  return db
    .prepare("SELECT COUNT(*) AS n FROM case_evidence WHERE case_id = ?")
    .get(caseId).n;
}

export function getEvidenceById(id) {
  return (
    db.prepare("SELECT * FROM case_evidence WHERE id = ?").get(id) ?? null
  );
}
...
function rowToUser(row) {
  ...
  return {
    ...
    // A user counts as "verified" once EITHER channel is confirmed.
    verified: !!row.phone_verified || !!row.email_verified,
...
// Prototype identity check (KYC demo). Records only a status + timestamp —
// the NID image and selfie never leave the browser and are never stored.
export function setUserKyc(userId, status) {
  db.prepare(
    "UPDATE users SET kyc_status = ?, kyc_verified_at = ? WHERE id = ?",
  ).run(status, status === "verified" ? now() : null, userId);
}
...
// `destination` is the phone number or email the code was sent to; `channel`
// records which so verification marks the right field.
export function saveOtp(userId, destination, codeHash, ttlMs, channel = "phone") {
  db.prepare(
    `INSERT INTO otp_codes (user_id, phone, code_hash, attempts, expires_at, channel)
     VALUES (?, ?, ?, 0, ?, ?)
     ON CONFLICT(user_id) DO UPDATE SET
       phone = excluded.phone, code_hash = excluded.code_hash,
       attempts = 0, expires_at = excluded.expires_at, channel = excluded.channel`,
  ).run(userId, destination, codeHash, new Date(Date.now() + ttlMs).toISOString(), channel);
}
...
// Amounts live server-side only — the client never decides what a tier costs.
export const TIER_AMOUNTS = { latte_large: 200, latte_regular: 100, hug: 0 };

export function addFeedback({ userId, supportType, rating, review, trxId = null }) {
  const id = randomUUID();
  const amount = TIER_AMOUNTS[supportType] ?? 0;
  db.prepare(
    `INSERT INTO feedback (id, created_at, user_id, support_type, rating, review, amount, trx_id, payment_status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    id,
    now(),
...
// Engineer checked the TrxID against the bKash statement — confirm it.
export function verifyFeedbackPayment(id) {
  db.prepare(
    "UPDATE feedback SET payment_status = 'verified' WHERE id = ? AND payment_status = 'claimed'",
  ).run(id);
}
